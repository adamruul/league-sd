...
var room_default = (onPeer, onSelfLeave) => {
  ...
  const makeAction = (type) => {
    ...
    if (typeEncoded.byteLength > typeByteLimit) {
      throw mkErr(
        `action type string "${type}" (${typeEncoded.byteLength}b) exceeds byte limit (${typeByteLimit}). Hint: choose a shorter name.`
      );
    }
...
var genKey = async (secret, ns) => crypto.subtle.importKey(
  "raw",
...
var joinRoom = initGuard(occupiedRooms, (config, ns) => {
  ...
  const trackerUrls = (config.trackerUrls || defaultTrackerUrls).slice(
    0,
    config.trackerUrls ? config.trackerUrls.length : config.trackerRedundancy || defaultRedundancy
  );
  if (!trackerUrls.length) {
    throw mkErr("trackerUrls is empty");
  }
  const infoHashP = crypto.subtle.digest("SHA-1", encodeBytes(`${libName}:${config.appId}:${ns}`)).then(
    (buffer) => Array.from(new Uint8Array(buffer)).map((b) => b.toString(36)).join("").slice(0, hashLimit)
  );
  const makeOffers = (howMany) => fromEntries(
  ...
  const onSocketMessage = async (socket, e) => {
    ...
    if (val.offer && val.offer_id) {
      ...
      peer.once(
        events.signal,
        async (answer) => socket.send(
          JSON.stringify({
            answer: key ? { ...answer, sdp: await encrypt(key, answer.sdp) } : answer,
    ...
    if (val.answer) {
      ...
      if (offer) {
        ...
        peer.signal(
          key ? { ...val.answer, sdp: await decrypt(key, val.answer.sdp) } : val.answer
        );
      }
    }
  };
  const announce = async (socket, infoHash) => socket.send(
    JSON.stringify({
      action: trackerAction,
      info_hash: infoHash,
      ...
      offers: await Promise.all(
        entries(offerPool).map(async ([id, { offerP }]) => {
          ...
          return {
            offer_id: id,
            offer: key ? { ...offer, sdp: await encrypt(key, offer.sdp) } : offer
          };
        })
      )
    })
...
*/
//# sourceMappingURL=trystero.js.map
